...
def parse_pdf(pdf_path: str) -> Script:
    ...
    # Load data-driven corrections once — shared by all parse paths.
    config = _load_corrections_config()

    # Block-level (PyMuPDF) parse — primary path on this branch.
    # Only falls through to pdfplumber if PyMuPDF is not installed.
    block_result = _block_parse(pdf_path, title, config)
    if block_result is not None:
        return block_result

    # pdfplumber fallback — only reached if PyMuPDF is unavailable.
    print("[parser] PDFPLUMBER FALLBACK (PyMuPDF not installed or no blocks found)",
          file=__import__("sys").stderr, flush=True)
